...
/**
 * The watchlist vocabulary. Stars and tags are one system, not two: a star is "I
 * want him", a tag is "here is why". Both live in the same inventory record.
 *
 * Carried over from the tagged-notes table in the old workbook, with "Breakout"
 * replaced by "Values" on 08/25/26 at James's word. Nothing had been tagged yet,
 * so no entry needed migrating.
 */
export const TAGS = ["Sleepers", "Values", "Busts", "My Guys", "Late Round Fliers"];

/**
 * The one-letter mark each tag wears when it is a toggle on a player row. Five
 * words do not fit beside a name 70 times over, and the OSD already says a
 * control's state by fill rather than by colour, so the letter only has to be
 * distinct. Defined here so the rail, the row and the focus card cannot drift.
 */
export const TAG_MARKS = {
  Sleepers: "S",
  Values: "V",
  Busts: "B",
  "My Guys": "G",
  ...
  const branches = (plan?.branches || []).map((b, bi) => {
    const targets = b.picks.map((t, ti) => {
      ...
      return {
        ...
        index: ti,
        round: t.round,
        // The tag toggles on a plan row write to the inventory by player id, so
        // a resolved target has to carry one. A row James typed as a reminder
        // ("CHECK LATE ROUNDERS") resolves to nobody and stays id-less.
        id: p ? p.id : null,
