...
export function loadPlayers(dataDir, season) {
  ...
  return { players, byName, byId, byLastName };
}
...
function nearest(pool, norm) {
  ...
  // The bare name, no punctuation. Both callers wrap it in their own question
  // ("did they mean X?"), and baking a "?" in here made that read "X??" on the
  // unmatched banner. Formatting belongs to whoever renders it.
