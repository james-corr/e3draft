...
/**
 * Which cells hold keepers, as coordinates and nothing else.
 *
 * The keeper's NAME lives in the board grid like every other pick. House rule 2
 * keeps that file a grid of typed names, so the flag can't ride along in the
 * cell — and copying the name here too would give one fact two owners that
 * could drift apart. Coordinates alone can't drift: overwrite a keeper cell
 * with a different player and it is still that team's round-10 keeper slot.
 *
 * A marker pointing at a cell that has since been cleared renders nothing, so a
 * stale one is harmless and heals itself the next time the cell is written.
 */
function validateKeepers(input, current) {
  const teamCount = current.teams.length;
  const seen = new Set();
  const out = [];
  for (const k of Array.isArray(input) ? input : []) {
    const round = Number(k?.round);
    const teamIndex = Number(k?.teamIndex);
    if (!Number.isInteger(round) || round < 1 || round > current.rounds) continue;
    if (!Number.isInteger(teamIndex) || teamIndex < 0 || teamIndex >= teamCount) continue;
    const key = `${round}:${teamIndex}`;
    if (seen.has(key)) continue;
    seen.add(key);
    out.push({ round, teamIndex });
  }
  out.sort((a, b) => a.round - b.round || a.teamIndex - b.teamIndex);
  return out;
}
...
export function createStore(dataDir, season) {
  ...
  // The key was reserved as null long before anything wrote it. Normalise once
  // so every reader downstream can just iterate.
  league.keepers = validateKeepers(league.keepers, league);
  ...
  return {
    ...
    saveLeague(next) {
      ...
      // Keeper markers travel with their column, exactly as picks do. `order` is
      // old-index-per-new-slot, so a keeper held by the team that was in column
      // `t` now belongs at whichever new slot points back at `t`. Without this a
      // reorder would leave every K badge on the wrong manager.
      const order = Array.isArray(next?.order) ? next.order : null;
      const keepers = order
        ? validateKeepers(
            league.keepers.map((k) => ({ ...k, teamIndex: order.indexOf(k.teamIndex) })),
            league
          )
        : league.keepers;
      league = { ...league, teams: names, myTeam, keepers };
      writeJson(leaguePath, league);
      return league;
    },

    /** Set the full list of keeper cells. Coordinates only — see validateKeepers. */
    saveKeepers(list) {
      const keepers = validateKeepers(list, league);
      copyFileSync(leaguePath, `${leaguePath}.bak`);
      league = { ...league, keepers };
      writeJson(leaguePath, league);
      return keepers;
    },
